...
int proccess_scanf(const char *str, const char *format, va_list *args) {
    ...
    char buffer[256]; int len  = s21_strlen(start); s21_strncpy(buffer, start, len);
    int str_i = 0;
    ...
    while (*current) {
        ...
        if (*current == '%' && *(current + 1) != '\0') {
            ...
            else if (*current == 'n') { n_specifier(args, buffer, len); (void)*current++;}
